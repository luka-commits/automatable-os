...
**Two settings, and getting either wrong costs you the whole image. Both learned the hard way on 15.08.2026**, when a second attempt at this page's illustrations was called worse than the first and the older set was still on disk to compare against:

1. **`aspect_ratio: "16:9"`, never square.** Every slot on this page is a wide banner. A square source loses roughly half its height to `object-fit: cover` — and it is the half at the bottom, so a desk scene loses its desk and a document loses its page. Ask for "wide banner crop" in the prompt as well; the model honours it in the composition, not only in the frame.
2. **A scene, not an icon.** The prompt must say `not an icon` and then describe a place: *a dental practice reception at night, a softly glowing monitor showing a patient pipeline board, through the window the quiet street outside.* The failed attempt asked for a wrench, a balance scale and a door with a key — four competent line drawings that could sit on any page for any client, which is exactly the AI-stock look this whole visual language exists to avoid. Four scenes from the same practice on the same evening read as one world and belong to this client. Four symbols read as clip art.

The same holds for the hero, and it is `**Draw the benefit, not the mechanism**` above restated: the working version shows enquiries arriving from phone, laptop and search, flowing into one board, with the dentist and patient already in the treatment room. The failed one drew six clinic buildings wired to a magnifying glass — the mechanism, drawn precisely, saying nothing about what the client gets.

**For the dark scope tiles, spell out the ground:** *"deep warm dark brown, near `#1c1712`, filling the whole frame edge to edge. No white, no cream background, no light panel behind the subject"*, plus *"warm terracotta and soft amber glow, catching the edges. Everything else falls into the dark."* Without those two lines the model puts the subject on a light panel and the tile sits on the dark card like a sticker.

**Keep the prompt next to the image you kept.** Whatever your image tool writes them to, save the prompt beside the file. Before generating for a job you have illustrated before, look there first: rereading the prompt that worked is cheaper than a rerun and far better than remembering. That is how the failed second attempt above was diagnosed at all — the first set was still on disk with its prompts, and the difference was one line.
...
5. **Collapsible CV, also moved up here** — collapsed by default (`<details>`/`<summary>`, "More about my background"). Every line of it comes from `context/experience.md` through the flags `--stat`, `--trait`, `--client`, `--client-note`, `--background` and `--languages`; the numbers alone fall back to `context/config.yaml` when no `--stat` is given, because the screener refreshes those from the account itself. **Nothing in this block is hardcoded, and that is load-bearing, not stylistic:** it sat in the template as literal text until 15.08.2026, which meant every page anyone generated claimed one particular person's degree, employers and client list. A pitch page is a client-facing document, and a wrong fact in it is worse than a missing one. Whatever was not passed is left out, and if nothing was passed the whole section goes rather than opening onto an empty panel. Deliberately dense rather than minimal: up to five track-record numbers (earned, projects delivered, hours logged, Job Success Score, verified hourly rate), the client-sourced trait tags from Upwork's "Insights from completed jobs" (Collaborative, Committed to Quality, etc. — genuine third-party praise, not self-description), the real company names you have built for, and the employment/education background. When new verified facts land in `experience.md`, pull them in here too rather than leaving this section thin while richer data sits unused in the reference file.
...
through, and add nothing of your own. A missing section there means the flag is simply not
passed and that part of the page is left out — never fill the gap with something plausible.
`--profile-url` is read from `context/config.yaml` unless you override it.
