...
**Three things the generator cannot do for you, all measured on the example page (15.08.2026).** The model returns a square canvas whatever the subject is, and a square canvas is wrong for every slot on this page:

1. **Crop to the drawing.** The hero came back with 24% empty ground on top and 12% below, so at full width it was a screenful on its own and pushed the video — the thing the headline actually promises — off the first screen. Crop to the ink plus a small margin.
2. **Put the four scope images on one shared field.** The tile is a wide band with `object-fit: cover`, which keeps roughly the middle third of a square. That third ate the scale's beam and all but a corner of the key, leaving two of four images unreadable. Scale each drawing to fit a common landscape field (1040×400 worked) padded with the image's own ground colour: the tile then crops almost nothing, and four different shapes read as one set.
3. **Pull the ground onto `--bg` exactly.** The model's cream came back `#f4eee0` against the page's `#f3efe6` — close enough to look like a mistake rather than a choice, so the illustration sat there as a pasted rectangle. One per-channel gain over the whole image lands the ground on target without banding. `mix-blend-mode: multiply` does **not** solve this and was tried: cream over cream only darkens the rectangle.

None of this lives in `generate.py`, because it would mean adding Pillow to a repo that is deliberately dependency-free. It is three short throwaway scripts at generation time.
